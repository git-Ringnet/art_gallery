...
@section('content')
    <div class="p-6 fade-in">
        ...
        <div class="mb-6 flex justify-between items-center">
            <div>
                <h2 class="text-2xl font-bold text-gray-800 mb-2">
                    <i class="fas fa-database mr-2"></i>Quản Lý Database
                </h2>
                <p class="text-gray-600">Quản lý database</p>
            </div>
            ...
            <div class="flex items-center space-x-3">
                ...
                <select id="year-selector"
                    class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-semibold shadow-sm"
                ...
                @if($isViewingArchive)
                    <button onclick="resetToCurrentYear()"
                        class="inline-flex items-center px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors text-sm font-semibold shadow-md"
                        title="Quay lại xem dữ liệu năm hiện tại">
                        <i class="fas fa-undo mr-2"></i> Quay lại năm hiện tại
                    </button>
                @endif
            </div>
        </div>
        ...
        @if($isViewingArchive)
            <div
                class="mb-6 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-lg shadow-lg p-4 border-l-4 border-orange-700">
                <div class="flex items-center">
                    ...
                    <div class="ml-4 flex-1">
                        <h4 class="text-lg font-bold mb-1">
                            <i class="fas fa-eye mr-2"></i>Đang Xem Dữ Liệu Năm Cũ ({{ $selectedYear }})
                        </h4>
                    ...
                    <div class="flex-shrink-0 ml-4">
                        <button onclick="resetToCurrentYear()"
                            class="px-4 py-2 bg-white text-orange-600 rounded-lg hover:bg-gray-100 transition-colors font-semibold shadow">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
                <div class="flex items-center justify-between mb-2">
                    <h5 class="text-gray-700 font-semibold">
                        <i class="fas fa-calendar-check mr-2"></i>Năm Hiện Tại
                    </h5>
                </div>
                <h2 class="text-3xl font-bold text-blue-600 mb-2">{{ $currentYear->year ?? 'N/A' }}</h2>
                <p class="text-sm text-gray-600">Database: <code
                        class="bg-gray-100 px-2 py-1 rounded">{{ $currentYear->database_name ?? 'N/A' }}</code></p>
            </div>

            <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-cyan-500">
                <div class="flex items-center justify-between mb-2">
                    <h5 class="text-gray-700 font-semibold">
                        <i class="fas fa-eye mr-2"></i>Đang Xem
                    </h5>
                </div>
                <h2 class="text-3xl font-bold text-cyan-600 mb-2">{{ $selectedYear }}</h2>
                @if($isViewingArchive)
                    <span class="inline-block bg-orange-100 text-orange-700 text-xs px-3 py-1 rounded-full font-semibold">Đang
                        xem dữ liệu cũ</span>
                @else
                    <span class="inline-block bg-green-100 text-green-700 text-xs px-3 py-1 rounded-full font-semibold">Dữ liệu
                        hiện tại</span>
                @endif
            </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-green-500">
                <div class="flex items-center justify-between mb-2">
                    <h5 class="text-gray-700 font-semibold">
                        <i class="fas fa-server mr-2"></i>Trên Server
                    </h5>
                </div>
                <h2 class="text-3xl font-bold text-green-600 mb-2">{{ $availableYears->count() }}</h2>
                <p class="text-sm text-gray-600">database có sẵn</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-blue-500 to-cyan-600 px-6 py-4">
                <h5 class="text-white font-semibold text-lg">
                    <i class="fas fa-list mr-2"></i>Danh Sách Database
                </h5>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Năm
                            </th>
                        ...
                        @forelse($allYears as $year)
                            <tr class="{{ $year->is_active ? 'bg-blue-50' : 'hover:bg-gray-50' }}">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <strong class="text-gray-900">{{ $year->year }}</strong>
                                        @if($year->is_active)
                                            <span
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <code class="bg-gray-100 px-2 py-1 rounded text-sm">{{ $year->database_name }}</code>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @else
                                        <span class="text-gray-400 text-sm">Chưa export</span>
                                    @endif
                                </td>
                                    ...
                                    <div class="flex space-x-2">
                                        @if($year->is_on_server)
                                            @if($selectedYear != $year->year)
                                                <button
                                                    class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none transition-colors switch-year"
                                                    data-year="{{ $year->year }}" title="Chuyển sang xem dữ liệu năm {{ $year->year }}">
                                                    <i class="fas fa-eye mr-1"></i> Xem
                                                </button>
                                            @else
                                                <span
                                                    class="inline-flex items-center px-3 py-1.5 rounded-md text-xs font-medium bg-cyan-100 text-cyan-800">
                                            ...
                                            <button
                                                class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none transition-colors export-db"
                                                data-year="{{ $year->year }}"
                                            ...
                                            <button
                                                class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none transition-colors view-exports"
                                                data-year="{{ $year->year }}" title="Xem danh sách file export đã tạo">
                                                <i class="fas fa-list mr-1"></i> Files
                                            </button>
                                        @else
                                            <span
                                                class="inline-flex items-center px-3 py-1.5 rounded-md text-xs font-medium bg-gray-100 text-gray-500"
                                        ...
                                        <button
                                            class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-yellow-600 hover:bg-yellow-700 focus:outline-none transition-colors import-db"
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                <div>
                    <h6 class="font-semibold text-gray-800 mb-2">1. Export Database:</h6>
                    <p class="text-gray-600 text-sm">Click nút "Export" → Nhập mô tả → File sẽ được tạo và lưu vào hệ thống
                    </p>
                </div>
                <div>
                    <h6 class="font-semibold text-gray-800 mb-2">2. Xem File Export:</h6>
                    <p class="text-gray-600 text-sm">Click nút "Files" → Xem danh sách → Click "Tải" để download</p>
                </div>
                <div>
                    <h6 class="font-semibold text-gray-800 mb-2">3. Import Database:</h6>
                    <p class="text-gray-600 text-sm">Click nút "Import" → Chọn file SQL → Xác nhận import</p>
                </div>
                <div>
                    <h6 class="font-semibold text-gray-800 mb-2">4. Xem Dữ Liệu Năm Cũ:</h6>
                    <p class="text-gray-600 text-sm">Click nút "Xem" → Dữ liệu chỉ đọc, không thể sửa/xóa</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="exportsModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-10 mx-auto p-5 border w-4/5 max-w-4xl shadow-lg rounded-md bg-white">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-900">Danh Sách File Export - Năm <span id="exports_year"></span>
                </h3>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Ngày Export</th>
